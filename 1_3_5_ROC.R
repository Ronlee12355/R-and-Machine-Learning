...
#pred.auc1<-performance(preed1,"tpr","fpr")
pred.auc1<-performance(preed1,"auc")
auc1[i]<-unlist(pred.auc1@y.values)
#plot(pred.auc1,type="l",lty=1)
}
#1：3将数据分类
index3<-sample(nrow(data_nav),nrow(data_pos)*3)
data_clean_3<-rbind(data_nav[index3,],data_pos)
...
for(i in 1:50){
  ...
  #pred.auc5<-performance(preed5,"tpr","fpr")
  pred.auc5<-performance(preed5,"auc")
  auc5[i]<-unlist(pred.auc5@y.values)
  #plot(pred.auc1,type="l",lty=1,lwd=6)
  #abline(a=0,b=1,lwd=2,lty=2)
  #plot(pred.auc3,lty=1,lwd=6,type="l")
  #plot(pred.auc5,lty=1,lwd=6,type="l")
}
